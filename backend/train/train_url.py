...
def load_url_dataset(logger):
    ...
    try:
        ...
        logger.info("URL dataset loaded successfully")
        return df
...
def engineer_features(df):
    ...
    df['domain_length'] = df['url'].apply(lambda x: len(x.split('/')[0]) if '/' in x else len(x))
    df = df.drop('url', axis=1)
    return df

def train_url_model():
    """Train URL model with multiple algorithms and hyperparameter tuning"""
    logger = setup_logging('train_url')
    timer = TrainingTimer(logger)

    timer.start()
    logger.info("Starting URL model training")
    ...
    generate_report('train_url', best_name, ['url'], time.ctime(start_time), time.ctime(end_time), timer.elapsed(), metrics, model_path, graph_paths)

    logger.info("URL model trained and saved successfully")
